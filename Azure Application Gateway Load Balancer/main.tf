...
module "appgateway" {
  ...
  resource_group_name = var.resource_group_name
  location =var.location
  # Pass subnet + public IP from network module
  appgw_subnet_id = module.network.appgw_subnet_id
  appgw_pip_id = module.network.appgw_pip_id
  # Pass NICs from network module for backend pool association
  network_interface_ids = module.network.virtual_network_interfaces_ids
    nic_pool_mapping = [
    { nic_index = 0, pool_name = "ImageServerPool" },
    { nic_index = 1, pool_name = "VideoServerPool" },
  ]
  depends_on = [module.network,module.machines]
}
